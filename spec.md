...
`List`s are encoded as 64 bit length (big endian encoded) followed by each
element concatenated.

A `String` is a `List` of bytes containing the UTF-8 encoded character string.

`Word32` is a 32 bit unsigned integer that is encoded in big endian.

`Node Info` is encoded in the [packed node
...
The types `Word32` and `String` have no special deconstructed type and are just
used as-is.
...
Thus, the actual length is `12 + 1 + n` where `n` is the length of the data
format name.
...
Output: The binary encoding of the [deconstructed
value](#deconstructed-values). On decoding failure, this test must return
`Failure`. If the SUT incorrectly determines that the byte array was a correct
encoding of the data type, the test fails.
...
ll Length & Value & When  
`1` & 0x00 & Base key == Node key  
`2` & 0x01, `n` & otherwise

The value of `n` is the k-bucket index of the Node key in a k-buckets instance
