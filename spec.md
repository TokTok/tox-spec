...
the entries are sorted by [\#distance](#distance)distance from the base key.
Thus, the first (smallest) element of the set is the closest one to the base
key in that set, the last (greatest) element is the furthest away.
...
If a bucket is full (i.e. the bucket contains more entries than the configured
maximum bucket size) when adding a node, the greatest entry is removed to keep
the bucket size below the maximum size.
